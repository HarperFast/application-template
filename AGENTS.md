...
- The schema.graphql and resources.js define the endpoints for your application through the @export directive and the module's exports. Harper does not have any additional routing APIs, and do not attempt to add use custom routing APIs (from other frameworks) that do not exist in Harper.
- Ensure that use the `static loadAsInstance = false` option in resources defined in resources.js.
